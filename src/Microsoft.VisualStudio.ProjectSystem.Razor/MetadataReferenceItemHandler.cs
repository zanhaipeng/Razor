...
namespace Microsoft.VisualStudio.ProjectSystem.Razor
{
    ...
    internal class MetadataReferenceItemHandler : ICommandLineHandler
    {
        ...
        public void Handle(IComparable version, BuildOptions added, BuildOptions removed, bool isActiveContext, IProjectLogger logger)
        {
            ...
            var outputDirectory = Path.GetDirectoryName(_context.BinOutputPath);
            for (var i = 0; i < removed.MetadataReferences.Length; i++)
            {
                var reference = removed.MetadataReferences[i];
                if (Path.GetDirectoryName(reference.Reference) == outputDirectory)
                {
